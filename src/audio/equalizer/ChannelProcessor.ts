...
export class ChannelProcessor {
  ...
  public constructor(bandMultipliers: number[]) {
    this.history = new Array(Equalizer.BAND_COUNT * 6).fill(0) as number[];
  ...
  public processInt(int: number): number {
    let result = int * 0.25;
  ...
  public process(samples: Buffer, extremum = 131072, bytes = 2, readInt?: ReadIntCallback, writeInt?: WriteIntCallback): Buffer {
