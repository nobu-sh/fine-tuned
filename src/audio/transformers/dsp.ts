...
export function applyPulsator(config: AFPulsatorConfig, int: number, channel: LR): number {
...
export function applyTremolo(config: AFTremoloConfig, int: number, sampleRate: number): number {
...
// TODO
export function applyVibrato(config: AFVibratoConfig, int: number, sampleRate: number): number {
  const fOffset = 1.0 - config.depth;
  const modSignal = fOffset + config.depth * Math.sin(config.phase);
  config.phase += ((2 * Math.PI) / sampleRate) * config.frequency;
  return modSignal * int;
...
export function applyVolume(vol: number, int: number): number {
  return vol * int;
}
