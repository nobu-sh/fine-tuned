...
export class BiquadFilter {
  ...
  public setFilter(filter: BiquadFilters, options: BiquadSetFilterProps): void {
    const coefficients = Coefficients.from(filter, options.fs, options.f0, options.Q, options.gain);

    this.update(coefficients);
  }

  public update(coefficients: Coefficients): void {
    this.coefficients = coefficients;
  }

  public replace(coefficients: Coefficients): void {
    this.coefficients = coefficients;
  }
  ...
  public runTransposed(input: number): number {
    const { a1, a2, b0, b1, b2 } = this.coefficients;
